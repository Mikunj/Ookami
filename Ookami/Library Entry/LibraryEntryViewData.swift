...
extension LibraryEntryViewData {
    ...
    //Increment the value of data at the given index path if it can be incremented
    func incrementValue(at indexPath: IndexPath) {
    ...
    //Edit the data at a given index path
    func editData(at indexPath: IndexPath, tableView: UITableView) {
        let tableData = self.tableData()
        let data = tableData[indexPath.row]
        ...
        case .status:
            ...
            ActionSheetStringPicker.show(withTitle: "Status", rows: rows, initialSelection: initial, doneBlock: { picker, index, value in
                self.update(status: statuses[index])
                tableView.reloadData()
            }, cancel: { _ in }, origin: tableView)
        ...
        case .rating:
            ...
            ActionSheetStringPicker.show(withTitle: "Rating", rows: rows, initialSelection: initial, doneBlock: { picker, index, value in
                ...
                self.update(rating: Double(index) / 2)
                tableView.reloadData()
            }, cancel: { _ in }, origin: tableView)
        ...
        case .reconsumeCount:
            ...
            ActionSheetStringPicker.show(withTitle: "Reconsume Count", rows: rows, initialSelection: unmanaged.reconsumeCount, doneBlock: { picker, index, value in
                if let newValue = value as? Int {
                    self.update(reconsumeCount: newValue)
                    tableView.reloadData()
                }
            }, cancel: { _ in }, origin: tableView)
            break
        ...
        case .reconsuming:
            ...
            self.update(reconsuming: !unmanaged.reconsuming)
            tableView.reloadData()
            break
            
        case .isPrivate:
            self.update(isPrivate: !unmanaged.isPrivate)
            tableView.reloadData()
            break
        }
    }
